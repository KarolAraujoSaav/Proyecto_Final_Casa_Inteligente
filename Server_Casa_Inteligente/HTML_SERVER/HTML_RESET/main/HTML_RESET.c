...
esp_err_t set_get(httpd_req_t *req) {
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {
        ...
        if (httpd_query_key_value(query, "reset", reset_str, sizeof(reset_str)) == ESP_OK) {

            // Cambiar estado sistema.
...
esp_err_t state_get(httpd_req_t *req) {
    ...
    // Convertir estado reset a texto.
    if (sistema_estado == SISTEMA_RESET) {
    ...
    sprintf(
        resp,
        "{\"puerta\":\"%s\",\"luz\":\"%s\",\"brillo\":%d,\"reset\":\"%s\"}",
        puerta_txt,
        luz_txt,
        brillo_luz,
        reset_txt
